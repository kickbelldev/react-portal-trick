...
interface PortalState {
  targets: Map<PortalMode, HTMLElement>  // 'main' | 'mini' → DOM ref
  mode: PortalMode                       // 현재 활성 슬롯
  returnPath: string | null              // 미니플레이어에서 돌아갈 경로
}

interface PortalActions {
  register: (mode, target) => void    // PortalSlot이 호출
  unregister: (mode) => void          // PortalSlot cleanup에서 호출
  setMode: (mode) => void             // MainPortal이 호출
  setReturnPath: (path) => void
  reset: () => void
}
```
...
│   ├── portal/
│   │   ├── model/
│   │   │   └── store.ts           # targets Map, mode, returnPath
│   │   └── components/
...
│   └── player/
│       ├── model/store.ts         # videoRef, src, isPlaying
│       └── components/
